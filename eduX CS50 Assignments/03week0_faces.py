...
# and prints the result. You’re welcome, but not required, to prompt the user explicitly, 
# as by passing a str of your own as an argument to input. Be sure to call main at the bottom of your file 

#--------------------------------------------------------

def main ():
    inputVar = input("What do you have to say : ")
    outputVar= convert(inputVar)
    print (outputVar)


def convert(to):
    outputVar = to.replace(":(","🙁" ).replace(":)","🙂")
    return outputVar
